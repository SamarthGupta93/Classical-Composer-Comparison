...
def midiToNoteStateMatrix(midifile):
    ...
    #state = [[0,0] for x in range(span)] # Originial
    state = [0 for x in range(span)]
    #statematrix.append(state)
    while True:
        # Why are we capturing at 60,180,300,... instead of 120, 240, 360, 480? 
        if time % (pattern.resolution / 4) == (pattern.resolution / 8):
            # Crossed a note boundary. Create a new state, defaulting to holding notes
            oldstate = state
            #state = [[oldstate[x][0],0] for x in range(span)] # Originial
                ...
                if isinstance(evt, midi.NoteEvent):
                    if (evt.pitch < lowerBound) or (evt.pitch >= upperBound):
                        ...
                        if isinstance(evt, midi.NoteOffEvent) or evt.velocity == 0:
                            #state[evt.pitch-lowerBound] = [0, 0] # Originial
                            state[evt.pitch-lowerBound] = 0
                        else:
                            #state[evt.pitch-lowerBound] = [1, 1] # Originial
